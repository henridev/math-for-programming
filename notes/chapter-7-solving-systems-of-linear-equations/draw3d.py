...
class Polygon3D():
    def __init__(self, *vertices, color=blue, plane=False):
...
def draw3d(*objects, origin=True, axes=True, width=6, save_as=None, azim=None, elev=None, xlim=None, ylim=None, zlim=None, xticks=None, yticks=None, zticks=None, depthshade=False, annotations=None, annotation_size=10):
    ...
    for object in objects:
        if type(object) == Points3D:
            ...
            if object.plane:
                # verts = [[xx, yy, zz] for xx, yy, zz in zip(*plane_1)]     # NEW
                ax.add_collection3d(Poly3DCollection(object.vertices, facecolors=['r', 'g', 'b', 'y'], alpha=.6))
            else:
    ...
    if annotations:
        for (annotation, x, y, z) in annotations:
            ax.text(x, y, z, annotation, size=annotation_size, zorder=1, color='k')
